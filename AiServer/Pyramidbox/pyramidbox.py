...
class PyramidBox(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer2 = nn.Sequential(self.resnet.layer2)
        self.layer3 = nn.Sequential(self.resnet.layer3)
        self.layer4 = nn.Sequential(self.resnet.layer4)
        self.layer5 = nn.Sequential(
            *[nn.Conv2d(out_c, 512, kernel_size=1),
              nn.BatchNorm2d(512),
    ...
    def forward(self, x):
        ...
        conv3_3 = self.layer1(x)
        conv4_3 = self.layer2(conv3_3)
        conv5_3 = self.layer3(conv4_3)
        convfc_7 = self.layer4(conv5_3)
        conv6_2 = self.layer5(convfc_7)
        conv7_2 = self.layer6(conv6_2)
...
lfpn_cpm_cfg = [256, 512, 1024, 2048, 512, 256]
# lfpn_cpm_cfg = [256, 512, 1024, 512, 512, 256]
...
def build_net(phase, num_classes=2, **kwargs):
    base_, extras_, head_ = multibox(
        vgg(vgg_cfg, 3), add_extras((extras_cfg), 1024))
    lfpn_cpm = add_lfpn_cpm(lfpn_cpm_cfg)
    return PyramidBox(phase, base_, extras_, lfpn_cpm, head_, num_classes, **kwargs)
